            ...
            @if(count($result))
                @foreach($result as $res)
                    <tr>
                        ...
                        <td><img class="bimg-x" src="{{url($res->bimgx)}}" alt=""></td>
                        <td>{{$res->bimgb}}</td>
                        <td>{{$res->bimgt}}</td>
                        <td>
                            <p><a style="cursor:pointer;" class="bimgtrue">通过</a></p>
                            <input type="hidden" value="{{$res->id}}">
                            <p><a style="cursor:pointer;" class="bimgfalse">删除</a></p>
                        </td>
                    </tr>
                @endforeach
            @else
...
@section('addjs')
    <script>
        $(function(){
            ...
            //删除审核作品
            $('.bimgfalse').each(function(index){
                $(this).on('click',function(){
                    $current=$(this).parent().parent().parent();
                    $work_id=$(this).parent().prev().val();
                    $.ajax({
                        url:'/admin/user/bimgfalse/'+$work_id,
                        type:'get',
                        success:function(data){
                            if(data){
                                alert('删除~~');
